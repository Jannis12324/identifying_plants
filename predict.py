...
parser.add_argument("image_path", metavar = "image_path", default = "flowers/test/28/image_05214.jpg", help = "The path to a single image.")
parser.add_argument("checkpoint", metavar = "checkpoint", default = "checkpoint.pth", help = "The checkpoint file which should be used to build the model.")
...
# Load cat to name conversion file

# Load the model
model, criterion, optimizer = load_checkpoint(args.checkpoint, args.gpu, args.verbose)

# predict
predict(args.image_path, model, args.top_k, args.gpu, args.category_names, args.verbose)
